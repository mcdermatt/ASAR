...
#     inputs = tf.keras.Input(shape=(3 + 3*2*L_embed)) #old (embed everything together)
    inputs = tf.keras.Input(shape=(6 + 3*2*(4) + 3*2*(10))) #new (embedding dims (4) and (10) )
    # outputs = inputs #old
    outputs = inputs[:,:(3+3*2*(10))] #only look at positional stuff for now

    #try removing view dependant effects from density 

    for i in range(D):
        outputs = dense()(outputs)
        #test-- add layer normalization
        outputs = tf.keras.layers.LayerNormalization()(outputs)
        
        if i%4==0 and i>0:
            outputs = tf.concat([outputs, inputs[:,:(3+3*2*(10))]], -1)

    #extend small MLP after output of density channel to get ray drop
    sigma_channel = dense(1, act=None)(outputs)    
    rd_start = tf.concat([outputs, inputs[:,(3+3*2*(10)):]], -1)
#     rd_channel = dense(128, act=relu)(outputs) #was this
    rd_channel = dense(256, act=relu)(outputs) #test matching OG NeRF structure
...
#     #only seems to draw well with single vertical patch
...
#     #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    encoded_ray_pos = embed_fn(ray_pos_flat, 10) #10 embedding dims for pos
