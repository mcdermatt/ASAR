...
""" Script to generate Monte-Carlo results for experiment 3 of 3D-ICET paper """

#NOTE: to remove extended axis removal operation, un-comment out <axislen_actual = 0.1*tf.math....> in get_U_and_L_cluster()>

niter = 500

c1_raw = np.loadtxt("plane_scan1.txt", dtype = float)
# c2_raw = np.loadtxt("plane_scan2.txt", dtype = float)
c2_raw = np.loadtxt("plane_scan1.txt", dtype = float) #use same scan as first frame
...
for i in range(niter):
	...
	#translate scan2 
	random_translation = 0.125*tf.constant([np.random.randn(), np.random.randn(), np.random.randn()])
...
# #with mitigation
# np.save("MC_results/scene3_ICET_estimates_v1", ICET_estimates)
# np.save("MC_results/scene3_ICET_pred_stds_v1", ICET_pred_stds)

#no mitigation
np.save("MC_results/scene3_ICET_estimates_NM_v1", ICET_estimates)
np.save("MC_results/scene3_ICET_pred_stds_NM_v1", ICET_pred_stds)
