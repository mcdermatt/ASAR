...
#set labels on top
ax[0,0].set_title("Small Object")
ax[0,1].set_title("Voxel Sized Object")
ax[0,2].set_title("Object Extends Over Multiple Voxels")
#show labels on side
...
#sign
subdivide_scan(pp0_low[np.squeeze(np.argwhere(g0hit))], fig, ax[1,0], fidelity = fid, min_num_pts = 5, flag = True)
subdivide_scan(pp0_high[np.squeeze(np.argwhere(g0hit2))], fig, ax[2,0], fidelity = fid, min_num_pts = 5, flag = True)
...
#wall
subdivide_scan(pp2_low[np.squeeze(np.argwhere(g2hit))], fig, ax[1,2], fidelity = fid, min_num_pts = minpts, flag = True)
subdivide_scan(pp2_high[np.squeeze(np.argwhere(g2hit2))], fig, ax[2,2], fidelity = fid, min_num_pts = minpts, flag = True)
...
file_location = "assets/3x3v2.png"
fig.set_size_inches(10,10)
fig.savefig(file_location)
time.sleep(1)

temp = cv2.imread(file_location, cv2.IMREAD_GRAYSCALE)
# gray = cv2.cvtColor(temp, cv2.COLOR_BGR2GRAY)
cv2.imshow('Grayscale', temp)
# time.sleep(1)
cv2.waitKey(3000)
cv2.imwrite(file_location, temp)