...
PP_ins_last = PP;               % init matrix that stores value of PP the last time fusion was performed
PM_combined = zeros(21,21);               %init PM_combined value to PP
Ncol = 22;
...
%     221745 % from manual inspection of LX code...
    ...
    [lla_ins, ned_lidar, lla_combined, vel, rpy, dv, qbn, xHatM_ins, xHatM_lidar, xHatM_combined, PP, gpsUpdated, lidarUpdated, gpsCnt, lidarCnt, gpsResUpd, PM_ins, PM_lidar, PM_combined, F200] = ...
        ins_gps(lla0_ins, ned0_lidar, lla0_combined, lla_ins_last, vel0, rpy0, dv0, qbn0, qbn_lidar, msrk, msrk1, gpsmsr, lidarmsr, xHatM_ins, xHatM_lidar, xHatM_ins_cum, xHatM_lidar_cum, PP, PP_ins_last, PM_combined, gpsCnt, lidarCnt, t_last_lidar, PM_lidar_last, fuse_lidar, fuse_gps, F200);
...
%             vel = (dpos_lidar(lidarCnt, 2:4)*10 + vel)/2 ;        
%             vel = [dpos_lidar(lidarCnt, 3)*10, dpos_lidar(lidarCnt, 2)*10, dpos_lidar(lidarCnt, 4)*10];
            %-------------------
    ...
    if gpsUpdated == 1
        ...
        PP_ins_last = PM_ins; %DEBUG: should this be PP_ins_last = PM_combined???
%         PP_ins_last = PM_combined;
    else
...
%     xHatM_lidar = [denu_lidar_scaled].'; %was this -just looking at changes in pos (no delta_rotation)
    ...
    PM_lidar = PM_lidar_last + Qk_lidar; %incorrect
%     PM_lidar = PM_ins_last + Qk_lidar
...
%         dt_lidar = gpstime - t_lidar;
        ...
        %NOTE 1/20: we should be doing WLS on the NEW error added between
        % INS and lidar since the last update (since we are adding up the
        % new contributions anyways)??
        
       %for debug---------
%         'PM_ins'
%         PM_ins(1,1) %this should be larger
%         'PP_ins_last'
%         PP_ins_last(1,1) %this should be smaller (since P should grow over time)
    
%         PM_ins(1,1) - (PP_ins_last(1,1) + 10*Qk_lidar(1,1))
        %------------------        

          % was this (per Jason's instructions)
%         W = pinv([PM_ins, F200*PP_ins_last;
%                   PP_ins_last*(F200.'), PP_ins_last + [10*Qk_lidar, zeros(3,18); zeros(18,21)]]);   
        W = pinv([PM_ins, zeros(21,21);
                  zeros(21,21), PP_ins_last + [10*Qk_lidar, zeros(3,18); zeros(18,21)]]);
        H = [eye(21);  eye(3), zeros(3,18); zeros(18,21)];

        %TEST: weight based on CHANGE IN ERRROR between GPS keyframes
%         W = pinv([(PM_ins - PP_ins_last), zeros(21,3);
%                    zeros(3,21), 10*Qk_lidar], 1e-20);
%         H = [eye(21); eye(3), zeros(3,18)];
              
        PM_combined = pinv(H.' * W * H);
%         PM_combined(1,1)
%         PM_combined = PM_ins + PM_lidar;
        PM_combined = (PM_ins + [PM_lidar, zeros(3,18); zeros(18,21)])/2; %for debug
...
% %         lla_ins = pinv((H.')*W*H)*(H.')*W*[lla_ins.'; zeros(18,1); lla_lidar.'];
% %         %resize to 3x1
%         lla_ins = lla_ins(1:3).';
% %         lla_ins

        %TODO: interpolate qbn's with SLERP
%          ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
