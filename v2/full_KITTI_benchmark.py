...
for i in range(start, num_frames):
	...
	# # lim1 = 50
	# lim2 = -10 #ignore points less than -10m in height (exist purely from reflections)
	...
	# body_frame_delta = dataset.poses[i][:3,:3].T.dot( dataset.poses[i+1][:3,3]) - dataset.poses[i][:3,:3].T.dot( dataset.poses[i][:3,3]) 
	...
	#doesn't work
	# angs = R2Euler(dataset.poses[i+1][:3,:3] - dataset.poses[i][:3,:3])
	# true_pose[i,4] = angs[0]
	# true_pose[i,5] = angs[1]
	# true_pose[i,3] = angs[2]



# np.savetxt("ICET_benchmark_05.txt", ICET_estimates)
# np.savetxt("ICET_pred_stds_05.txt", ICET_pred_stds)
np.savetxt("true_pose_005.txt", true_pose)
