...
int main(int argc, char** argv) {
    ...
    int numBinsPhi = 24;  // Adjust the number of bins as needed
    int numBinsTheta = 75; // Adjust the number of bins as needed
    int n = 25; //50; // min size of the cluster
    float thresh = 0.1; // 0.1 indoor, 0.3 outdoor; // Jump threshold for beginning and ending radial clusters
    float buff = 0.1; // 0.1 indoor, outdoor 0.5; //buffer to add to inner and outer cluster range (helps attract nearby distributions)
    ...
    Eigen::VectorXf X(6); //global solution vector
            ...
            for (int theta = 0; theta< numBinsTheta; theta++){
                ...
                if ((indices2.size() > n) && (indices1.size() > n) && (clusterBounds.row(numBinsTheta*phi + theta)[5] > 1)) { //U and L won't exist if no useful bin in scan1
                    ...
                    if (filteredPoints2.size()/3 > n){
                        ...
                        c++;
                        //update for drawing (only on last iteration)
                        if (iter == (runlen-1)){
                            float alpha2 = 0.3f;
