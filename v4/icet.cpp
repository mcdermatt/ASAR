...
Eigen::VectorXf icet(Eigen::MatrixXf points, Eigen::MatrixXf points2, Eigen::VectorXf X0, 
    ...
    Eigen::VectorXf X(6); //solution vector
    ...
    for (int phi = 0; phi < numBinsPhi; phi++){
        for (int theta = 0; theta< numBinsTheta; theta++){
            ...
            if (indices.size() > n) {
                ...
                float innerDistance = clusterDistances.first;
                float outerDistance = clusterDistances.second;
    ...
    //apply initial transform to cloud2
